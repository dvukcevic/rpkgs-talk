...
3 blocks of text: Title, Description, Details

Document everything (functions, data, classes, methods,...)
...
> - List all such external packages under `Imports` in the `DESCRIPTION`  
Easy way: `use_package("package")`  
This will ensure these packages are installed when your own one is

> - If the package is not very essential, list under `Suggests` instead  
Easy way: `use_package("package", "Suggests")`  
This doesn't force users to install it, but alerts them to extra functionality
if they do
...
i.e. all functions except those with names starting with a period (`.`)

---

## Visibility to end users

Typically only want to provide a few high-level functions to users.

Use the `@export` tag to mark these:
...
Running `document()` will create appropriate entries in `NAMESPACE`:
```
export(resample)
```

(NOTE: this overrides the permissive default!)
